<template>
    <div class="flex lg:w-1/2 md:w-full sm:w-full xs:w-full  w-full ">
        <div class="inline-block mx-1 my-1 w-full sm:rounded shadow bg-grey-darker  text-white">
            <div class="flex items-center justify-between px-2 py-2" @click="storeMovement">
                ...
                <div class="float-right">
                        ...
                        <toggle-button :speed="80" :sync="true" :id="employee_id" :value="this.isOut()" :cssColors="true" class="rounded-full text-xl" :class="buttonClass" :height="40" :width="100" :labels="{checked: 'Out', unchecked: 'In'}"/>
                </div>
            </div>
        </div>
    </div>
